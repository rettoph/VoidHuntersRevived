...
namespace VoidHuntersRevived.Common.FixedPoint
{
    public struct FixVector2(Fix64 x, Fix64 y)
    {
        ...
        {
            return new((position.X * matrix.M11) + (position.Y * matrix.M21) + matrix.M41, (position.X * matrix.M12) + (position.Y * matrix.M22) + matrix.M42);
        }
        ...
        {
            return new((position.X * transform.Rotation.Real) + (position.Y * -transform.Rotation.Imaginary) + transform.Position.X, (position.X * transform.Rotation.Imaginary) + (position.Y * transform.Rotation.Real) + transform.Position.Y);
        }
