...
namespace VoidHuntersRevived.Common.FixedPoint
{
    public struct FixVector2(Fix64 x, Fix64 y)
    {
        ...
        public readonly FixPolar ToPolar() => new(
                length: this.Length,
                radians: Fix64.Atan2(this.X, this.Y));

        public readonly FixMatrix ToFixMatrix() => new(
        ...
        public static FixVector2 Lerp(FixVector2 v1, FixVector2 v2, Fix64 amount) => new(
        ...
        public static FixVector2 Transform(FixVector2 position, FixMatrix matrix) => new((position.X * matrix.M11) + (position.Y * matrix.M21) + matrix.M41, (position.X * matrix.M12) + (position.Y * matrix.M22) + matrix.M42);
        ...
        /// <returns>Transformed <see cref="Vector2"/>.</returns>
        public static FixVector2 Transform(FixVector2 position, FixTransform2D transform) => new((position.X * transform.Rotation.Real) + (position.Y * -transform.Rotation.Imaginary) + transform.Position.X, (position.X * transform.Rotation.Imaginary) + (position.Y * transform.Rotation.Real) + transform.Position.Y);
        ...
        public static void Transform(
            FixVector2[] sourceArray,
            ref FixMatrix matrix,
            FixVector2[] destinationArray) => Transform(sourceArray, 0, ref matrix, destinationArray, 0, sourceArray.Length);

        public XnaVector2 ToXna() => new()
        {
            X = (float)this.X,
            Y = (float)this.Y
        };
        ...
        public static FixVector2 FromPolar(Fix64 length, Fix64 radians) => new(
                x: Fix64.Cos(radians) * length,
                y: Fix64.Sin(radians) * length);

        public static FixVector2 Rotate(FixVector2 vector2, Fix64 radians) => FixVector2.FromPolar(vector2.Length, Fix64.Atan2(vector2.X, vector2.Y) + radians);

        public override readonly bool Equals(object? obj) => obj is FixVector2 vector &&
