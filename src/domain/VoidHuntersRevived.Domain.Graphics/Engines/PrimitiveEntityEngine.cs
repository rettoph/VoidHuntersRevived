...
namespace VoidHuntersRevived.Domain.Graphics.Engines
{
    ...
    ) : StrategyEngine, IReactOnAddEx<Primitive<TVertex>>, IOnDrawEngine, IQueryingEntitiesEngine
        where TVertex : unmanaged, IVertexType, IEntityComponent
    {
        ...
        public void Add((uint start, uint end) rangeOfEntities, in EntityCollection<Primitive<TVertex>> entities, ExclusiveGroupStruct groupID)
        {
            ...
            for (uint i = rangeOfEntities.start; i < rangeOfEntities.end; i++)
            {
                ref Primitive<TVertex> primitive = ref primitives[i];
                ...
                {
                    primitive = new Primitive<TVertex>(primitive.Type, teamSequenceGroup.Value);
                }
