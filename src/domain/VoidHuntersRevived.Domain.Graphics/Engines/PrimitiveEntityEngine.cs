...
namespace VoidHuntersRevived.Domain.Graphics.Engines
{
    ...
    ) : StrategyEngine, IReactOnAddEx<PrimitiveEntity<TVertex>>, IOnDrawEngine, IQueryingEntitiesEngine
        where TVertex : unmanaged, IVertexType, IEntityComponent
    {
        ...
        public void Add((uint start, uint end) rangeOfEntities, in EntityCollection<PrimitiveEntity<TVertex>> entities, ExclusiveGroupStruct groupID)
        {
            ...
            if (count == 0)
            {
                ...
                {
                    PrimitiveEntity<TVertex> primitive = primitives[i];
            ...
            for (uint i = rangeOfEntities.start; i < rangeOfEntities.end; i++)
            {
                ref PrimitiveEntity<TVertex> primitive = ref primitives[i];
                ...
                {
                    primitive = new PrimitiveEntity<TVertex>(primitive.Type, teamSequenceGroup.Value);
                }
