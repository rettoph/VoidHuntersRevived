...
namespace VoidHuntersRevived.Domain.Physics.Systems
{
        ...
        IPredictiveSynchronizationSystem
    {
        ...
        {
            Fix64 damping = step.ElapsedTime / (Fix64)0.5f;
            ...
            foreach (IBody lockstepBody in this._lockstepSpace.AllBodies())
            {
                ...
                FixVector2 targetPosition = FixVector2.SmoothStep(predictiveBody.Position, lockstepBody.Position, damping);
                Fix64 targetRotation = Fix64.SmoothStep(predictiveBody.Rotation, lockstepBody.Rotation, damping);

                FixVector2 targetLinearVelocity = FixVector2.SmoothStep(predictiveBody.LinearVelocity, lockstepBody.LinearVelocity, damping);
                Fix64 targetAngularVelocity = Fix64.SmoothStep(predictiveBody.AngularVelocity, lockstepBody.AngularVelocity, damping);
