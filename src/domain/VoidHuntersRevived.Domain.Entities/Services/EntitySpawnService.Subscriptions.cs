...
namespace VoidHuntersRevived.Domain.Entities.Services
{
        ...
        IEventEngine<HardDespawnEntity>
    {
        ...
        public void Process(VhId eventId, SoftSpawnEntity data)
        {
            ...
            if (exists == false)
            {
                _logger.Warning("{MethodName}, Entity not found. GlobalId = {GlobalId}, LocalId = {LocalId}", nameof(SoftSpawnEntity), data.GlobalId, localId);
                return;
            }

            if (status.IncrementSoftSpawnCount() == false || status.Value != EntityStatusEnum.HardSpawned)
            {
                _logger.Warning("{MethodName}, Invalid entity state. GlobalId = {GlobalId}, LocalId = {LocalId}, Status = {Status}, Count = {Count}", nameof(SoftSpawnEntity), data.GlobalId, localId, status.Value, status.Count);
                return;
            }
        ...
        public void InternalRevert(VhId eventId, SpawnEntity data)
        {
            ...
            if (exists == false)
            {
                _logger.Warning("{MethodName}, Entity not found. GlobalId = {GlobalId}, LocalId = {LocalId}", nameof(SpawnEntity), data.GlobalId, localId);
                return;
            }
        ...
        public void Revert(VhId eventId, DespawnEntity data)
        {
            ...
            {
                _logger.Warning("(Revert) {MethodName}, Invalid entity state. GlobalId = {GlobalId}, LocalId = {LocalId}, Status = {Status}, Count = {Count}", nameof(DespawnEntity), data.GlobalId, localId, status.Value, status.Count);
                return;
            }
            ...
            _logger.Verbose("(Revert) {MethodName}, Soft spawn. GlobalId = {GlobalId}, LocalId = {LocalId}", nameof(DespawnEntity), data.GlobalId, localId, status.Value);
        ...
        private void SoftDespawn(VhId sourceEventId, ref EntityStatus status, ref Entity entity, IEntityTemplate template)
        {
            ...
            {
                _logger.Warning("{MethodName}, SoftDespawn overflow. GlobalId = {GlobalId}, LocalId = {LocalId}", nameof(SoftDespawn), entity.GlobalId, entity.LocalId);
                return;
            }

            if (status.Value != EntityStatusEnum.SoftSpawned)
            {
                _logger.Warning("{MethodName}, Invalid state. GlobalId = {GlobalId}, LocalId = {LocalId}, Status = {Status}", nameof(SoftDespawn), entity.GlobalId, entity.LocalId, status.Value);
                return;
            }

            _logger.Verbose("{MethodName}, GlobalId = {GlobalId}, LocalId = {LocalId}, Template = {Template}", nameof(SoftDespawn), entity.GlobalId, entity.LocalId, template.Key.Name);
