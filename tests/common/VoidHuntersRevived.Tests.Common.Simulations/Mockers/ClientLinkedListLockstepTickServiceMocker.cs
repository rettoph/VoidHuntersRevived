...
{
    public class ClientLinkedListLockstepTickServiceMocker : BaseMockerBuilder<ClientLinkedListLockstepTickService>
    {
        ...
        public ClientLinkedListLockstepTickServiceMocker(int stepsPerTick)
        {
            this.SettingServiceMocker = new Mocker<ISettingService>()
                .Setup(x => x.GetValue(Settings.StepsPerTick), Settings.StepsPerTick.MockValue(stepsPerTick));
        }
