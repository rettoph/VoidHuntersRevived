...
namespace VoidHuntersRevived.Tests.Domain.FixedPoint
{
    public class FixTransform2DTests
    {
        ...
        public void FixTransform2D_Test(float x, float y, float rotation)
        {
            new FixTransform2D((Fix64)x, (Fix64)y, (Fix64)rotation)
                .AssetEqualTo(
        ...
        [InlineData(123, 45, MathF.PI)]
        [InlineData(123, 45, MathHelper.TwoPi)]
        public void FixTransform2D_Invert_Test(float x, float y, float rotation)
        {
            FixTransform2D.Invert(new((Fix64)x, (Fix64)y, (Fix64)rotation))
        ...
        public void FixTransform2D_Multiply_Test(float x, float y, float rotation, float tranX, float tranY)
        {
            (new FixTransform2D((Fix64)x, (Fix64)y, (Fix64)rotation) * new FixVector2(tranX, tranY))
                .AssetEqualTo(
                    Matrix.CreateRotationZ(rotation) * Matrix.CreateTranslation(x, y, 0) *
