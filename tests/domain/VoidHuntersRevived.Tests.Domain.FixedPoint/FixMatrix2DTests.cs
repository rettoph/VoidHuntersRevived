...
namespace VoidHuntersRevived.Tests.Domain.FixedPoint
{
    public class FixMatrix2DTests
    {
        ...
        public void FixMatrix2D_Multiply_Test(float x, float y, float rotation, float tranX, float tranY)
        {
            (new FixMatrix2D((Fix64)x, (Fix64)y, (Fix64)rotation) * new FixVector2(tranX, tranY))
                .AssetEqualTo(
                    Matrix.CreateRotationZ(rotation) * Matrix.CreateTranslation(x, y, 0) *
